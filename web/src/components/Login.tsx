  ...
  return (
    <div className="min-h-screen grid place-items-center px-4">
      <form onSubmit={submit} className="card max-w-sm w-full space-y-3">
        ...
        <button className="btn w-full" disabled={loading || !username || !password}>Log in / Create</button>
        <p className="text-xs text-gray-500">First login creates a user with this username (must be unique).</p>
      </form>
    </div>
  );
}
