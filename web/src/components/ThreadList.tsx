...
// List of conversation threads - clickable sidebar
    ...
    <div className="space-y-2 overflow-auto max-h-[calc(100vh-140px)]">
      {threads.map(t => (
        <button 
          ...
          className={`w-full text-left p-2 rounded ${activeId===t.id? 'bg-black text-white' : 'hover:bg-gray-100'}`}
        >
          {/* Show participant names */}
          <div className="text-sm font-medium">{t.participants.join(", ") || "(empty)"}</div>
          {/* Show thread ID and last message time */}
